...
#import "BLMCustomTopView.h"
#import "BLMCustomSearchView.h"
#import "BLMResultCityController.h"
#import "BLMCityModel.h"
#import "BLMCityTableViewCell.h"

@interface BLMCityViewController ()<CustomTopViewDelegate,CustomSearchViewDelegate,ResultCityControllerDelegate>
@property (nonatomic,retain) BLMCustomTopView *navView; //自定义导航栏
@property (nonatomic,retain) BLMCustomSearchView *searchView; //searchBar所在的view
...
@property (nonatomic,retain)BLMResultCityController *resultController;//显示结果的controller
//@property (nonatomic,retain)NSArray *currentCityArray;// 当前城市
@property (nonatomic,retain)NSArray *hotCityArray; // 热门城市
@end

@implementation BLMCityViewController
...
-(BLMResultCityController *)resultController
{
    if(!_resultController)
    {
        _resultController = [[BLMResultCityController alloc] init];
...
- (void)viewDidLoad {
    ...
    self.searchView = [[BLMCustomSearchView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH-20, 44)];
    ...
    self.navView = [[BLMCustomTopView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 64)];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.section<2)
    {
        ...
        BLMCityTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cityCell"];
        if(cell==nil)
        {
            cell = [[BLMCityTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cityCell" cityArray:self.dataArray[indexPath.section]];
        }
...
-(void)searchString:(NSString *)string
{
    ...
    if([self isZimuWithstring:string])
    {
        ...
        [array enumerateObjectsUsingBlock:^(NSString *cityName, NSUInteger idx, BOOL * _Nonnull stop) {
            BLMCityModel *model = [[BLMCityModel alloc] init];
